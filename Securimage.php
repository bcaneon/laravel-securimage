...
class Securimage
{
        ...
        if ($this->no_session != true) {   
            ...
            if (session()->has('securimage_code_value.'.$this->namespace) && trim(session()->get('securimage_code_value.'.$this->namespace)) != '') {
                ...
                if ($this->isCodeExpired(session()->get('securimage_code_ctime.'.$this->namespace)) == false) {                    
                    $code['code'] = session()->get('securimage_code_value.'.$this->namespace);
                    $code['time'] = session()->get('securimage_code_ctime.'.$this->namespace);
                    $code['display'] = session()->get('securimage_code_disp.'.$this->namespace);
                }
            }
        }
    ...
    */
    ...
    protected function validate()
    {
        ...
        if ($code != '') {
            ...
            if ((string)$code === (string)$code_entered) {
                ...
                if ($this->no_session != true) {
                    session(['securimage_code_disp.'.$this->namespace=> '']);
                    session(['securimage_code_value.'.$this->namespace=>'']);
                    session(['securimage_code_ctime.'.$this->namespace=>'']);
                    session(['securimage_code_audio.'.$this->namespace=>'']);
                }
    ...
    {
        if ($this->no_session != true) {
            if (session()->has('securimage_code_value') && is_scalar(session()->get('securimage_code_value'))) {
            ...
            session(['securimage_code_audio.'.$this->namespace=>null]);  
            session()->save();          
        }
